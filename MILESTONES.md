...
- `tray-icon` on Linux requires AppIndicator/GTK at runtime — document this dependency. <!-- done: README.md Linux X11 section, distro table; run_linux_tray() implemented in src/bin/comboauth-tray.rs using gtk::init + TrayIconBuilder + glib::MainContext::invoke for thread-safe quit -->
...
- Use `ashpd` global shortcuts portal where supported; degrade otherwise. <!-- done: src/hotkey/linux.rs spawn_listener/spawn_wayland_listener; ashpd 0.13 GlobalShortcuts portal with preferred_trigger <Control>k; degrades with warning on portal unavailability -->
- Fallback: copy secret to clipboard, send desktop notification ("Secret copied — paste manually"). <!-- done: notify_desktop() in src/paste/linux.rs spawns notify-send (Stdio::null, status-waited) before copy_and_clear -->
- Document Wayland limitations clearly in README. <!-- done: Wayland limitations section in README.md; table covering hotkey/paste/AT-SPI, honest current-status note (picker not yet wired), portal binding requirements, clipboard timing breakdown, notify-send dep -->
